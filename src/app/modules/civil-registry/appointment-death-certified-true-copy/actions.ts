...
export async function logDebugMessage(message: string) {
    console.log(`[LCR DECEASED APPT DEBUG] ${message}`);
}
...
export async function getBarangaysList() {
    try {
        const rows = await prisma.$queryRawUnsafe<{ name: string }[]>(
            `SELECT name FROM "BarangayInfo" ORDER BY name ASC`
        );
...
export async function ensureCivilRegistryTransactionTypes() {
    const types = [
        { code: "LCR_DEATH_PSA_APPOINTMENT_ENDORSEMENT", name: "Death PSA Appointment Endorsement", category: "Civil Registry", baseFee: 130 },
    ];
