...
export async function submitCivilRegistryTransaction(formData: FormData, userId: string) {
    try {
        ...
        if (app1BirthDate && calculateAge(app1BirthDate) < 18) {
            return { success: false, error: "Applicant 1 must be 18 years of age or older. Minors are not permitted to register a marriage." };
        }
        if (app2BirthDate && calculateAge(app2BirthDate) < 18) {
            return { success: false, error: "Applicant 2 must be 18 years of age or older. Minors are not permitted to register a marriage." };
        }
...
// ─── Get Existing Marriage Registrations ─────────────────────────────────────

export async function getExistingMarriageRegistrations(userId: string) {
    try {
        if (!userId) return { success: false, data: [] };

        const type = await prisma.transactionType.findFirst({
            where: { code: "LCR_MARRIAGE_REG" }
        });
        ...
        console.error("Error fetching existing marriage registration requests:", error);
        return { success: false, data: [] };
    }
}
