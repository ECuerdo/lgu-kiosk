...
export async function ensureCivilRegistryTransactionTypes() {
    const types = [
        { code: "LCR_MARRIAGE_LICENSE", name: "Marriage License Application", category: "Civil Registry", baseFee: 862 },
    ];
...
// ─── Get Existing Marriage Licenses ──────────────────────────────────────────

export async function getExistingMarriageLicenses(userId: string) {
    try {
        if (!userId) return { success: false, data: [] };

        const type = await prisma.transactionType.findFirst({
            where: { code: "LCR_MARRIAGE_LICENSE" }
        });
        ...
        console.error("Error fetching existing marriage license requests:", error);
        return { success: false, data: [] };
    }
}
