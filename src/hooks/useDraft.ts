...
function isDraftEmpty(obj: unknown): boolean {
    if (!obj) return true;
    ...
    for (const key in record) {
        if (Object.prototype.hasOwnProperty.call(record, key)) {
            ...
            if (key === "currentStep" && (record[key] === "GUIDE" || record[key] === "START")) continue;
            if ([
...
export function useDraft<T extends Record<string, unknown>>(draftKey: string) {
